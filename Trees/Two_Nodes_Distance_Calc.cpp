...
Node *findDistanceUtil(Node* root, int n1, int n2, int &d1,
                            int &d2, int &dist, int lvl)
{
    ...
    Node *left_lca  = findDistanceUtil(root->left, n1, n2,
                                   d1, d2, dist, lvl+1);
    Node *right_lca = findDistanceUtil(root->right, n1, n2,
                                   d1, d2, dist, lvl+1);
...
{
    int d1 = -1, d2 = -1, dist;
    Node *lca = findDistanceUtil(root, n1, n2, d1, d2,
                                          dist, 1);
